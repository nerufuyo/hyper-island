...
import com.nerufuyo.hyperisland.data.db.AppSetting
import com.nerufuyo.hyperisland.data.db.SettingsKeys
import com.nerufuyo.hyperisland.data.model.AppSettingBackup
import com.nerufuyo.hyperisland.data.model.BackupMetadata
...
class BackupManager(
    ...
    suspend fun performExport(uri: Uri, selection: BackupSelection): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            ...
            // 4. Write
            val backup = HyperIslandBackup(metadata, filteredSettings)
    ...
    suspend fun readBackupFile(uri: Uri): Result<HyperIslandBackup> = withContext(Dispatchers.IO) {
        try {
            val backup = context.contentResolver.openInputStream(uri)?.use { inputStream ->
                java.io.InputStreamReader(inputStream, Charsets.UTF_8).use { reader ->
                    gson.fromJson(reader, HyperIslandBackup::class.java)
                }
    ...
    suspend fun restoreBackup(backup: HyperIslandBackup, selection: BackupSelection): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
    ...
    companion object {
        fun generateFileName(): String {
            ...
            return "HyperIsland_Backup_$date.hbr"
        }
    }
}